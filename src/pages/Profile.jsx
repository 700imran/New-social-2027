...
import { Menu, Settings, MapPin, Calendar, LogOut, Pencil, Grid3x3, Clapperboard, Bookmark, Tags } from 'lucide-react'
    ...
    <div>
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-ink-100 bg-white/90 px-4 py-3 backdrop-blur">
        ...
        <div className="relative">
          <button onClick={() => setMenuOpen((m) => !m)} className="focus-ring rounded-full p-1.5 text-ink-700" aria-label="Menu">
      ...
      <div className="px-4">
        ...
        <div className="mt-4 flex items-center gap-6">
          ...
          <Stat label="Followers" value={currentUser.followers} onClick={() => navigate('/profile/followers')} />
          <Stat label="Following" value={currentUser.following} onClick={() => navigate('/profile/following')} />
        </div>

        <div className="mt-4">
          <button
            onClick={handleShare}
            className="w-full rounded-full bg-saffron-gradient py-2.5 text-sm font-bold text-navy-950 shadow-pop active:scale-[0.98]"
          >
            Share Profile
          </button>
        </div>
