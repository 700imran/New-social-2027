...
export default function Logo({ size = 32, withWordmark = false, wordmarkClass = '', variant = 'warm', className = '' }) {
  const gradId = variant === 'brand' ? 'logoGradBrand' : 'logoGrad'
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          ...
          {/* Used only when variant="brand" — the splash/welcome screens' warm-to-violet
              treatment (see LaunchSplash.jsx / Welcome.jsx). Kept separate from logoGrad
              above so every other existing screen's logo is untouched. */}
          <linearGradient id="logoGradBrand" x1="4" y1="4" x2="36" y2="36" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#FFC069" />
            <stop offset="45%" stopColor="#FF9933" />
            <stop offset="75%" stopColor="#C2508A" />
