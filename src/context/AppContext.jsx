...
export function AppProvider({ children }) {
  ...
  // in live mode) renders the same toggles/values a first-time live user
  // would see, rather than an empty or differently-shaped object.
  ...
  // back to the in-memory default above, even though the real values
  // were saved server-side the whole time.
  const fetchSettingsLive = useCallback(async () => {
    if (!isLive) return
    try {
      const settingsRow = await api.getSettings()
      setUserSettings(settingsRow)
    } catch {
      /* not fatal — Settings screens still work this session against the
         in-memory default, just won't reflect a previous session's saved
  ...
  //
  // Live mode hits the real, already-existing GET /users/:id/followers or
  // GET /users/:id/following (backend/src/routes/follows.js) — those return
  // bare {follower_id|followee_id, created_at} rows with no name/avatar
  // (the Level 1 API has no batch profile-lookup-by-id endpoint yet), so
  // this enriches each id via getUser()/cacheUsers the same way
  // ManagedAccounts.jsx already does for blocked/muted lists: known users
  // resolve to their real name; anyone this session hasn't seen yet falls
  // back to the same generic "BharatSpace user" placeholder used
  // everywhere else in the app for that exact situation — never invented.
  //
  // Mock mode has no stored follow graph for arbitrary users (only the
  // signed-in user's own `followedUserIds` is tracked locally), so:
  // viewing your OWN following list reflects that real, live state;
  // anyone else's followers/following falls back to a representative
  // slice of the seed USERS so the screen still demonstrates real UI
  // instead of always rendering empty — same "honest demo data" spirit
  // as INITIAL_POSTS itself.
  const getFollowList = useCallback(
    async (userId, kind) => {
      const targetId = userId || currentUser.id
      if (!isLive) {
        if (kind === 'following' && targetId === currentUser.id) {
          return [...followedUserIds].map((id) => getUser(id)).filter(Boolean)
        }
      ...
      try {
        const rows =
          kind === 'followers' ? await api.getFollowers(targetId) : await api.getFollowing(targetId)
        const ids = rows.map((r) => (kind === 'followers' ? r.follower_id : r.followee_id))
        return ids.map(
          (id) =>
            getUser(id) || { id, name: 'BharatSpace user', handle: '', avatarColor: avatarToneFor(id), initials: 'U' }
        )
      } catch {
        pushToast(`Could not load ${kind}`)
        return []
      }
    },
    [currentUser.id, followedUserIds, getUser, pushToast]
  )

  // Powers every "live" Settings screen (Notifications, Privacy & content
  // controls, Appearance, Focus mode, Words & comments). `partial` is
  // whatever subset of { notifications, privacy, appearance, focus,
  // muted_words } changed — e.g. { privacy: { isPrivate: true } } to flip
  // one field, not the whole settings object.
  //
  // Same optimistic-update-then-reconcile shape as toggleFollow above:
  // applied to local state immediately so a toggle switch feels instant,
  // then either confirmed by the server's response (live mode) or rolled
  // back with a toast if the save fails. Nested category objects
  // (notifications/privacy/appearance/focus) merge one level deep so
  // sending just `{ appearance: { theme: 'dark' } }` doesn't clobber
  // language/textSize already in state; muted_words is a plain array
  // replace, since callers always send the complete new list for that one.
    ...
    async (partial) => {
      ...
      setUserSettings((prev) => {
        ...
        for (const [key, value] of Object.entries(partial)) {
          next[key] = Array.isArray(value) ? value : { ...prev[key], ...value }
        }
        return next
      })

      if (!isLive) return // mock mode: local-only, same as most toggles elsewhere in this file
      ...
      try {
        ...
        pushToast('Could not save your settings — please try again')
      }
    },
    [userSettings, pushToast]
  )
