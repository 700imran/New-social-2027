...
export const addCloseFriend = (friendId) => request('/close-friends', { method: 'POST', body: { friendId } })
export const removeCloseFriend = (friendId) => request(`/close-friends/${friendId}`, { method: 'DELETE' })
export const getCloseFriends = () => request('/close-friends')
export const logSession = () => request('/account/sessions', { method: 'POST' })
export const getSessions = () => request('/account/sessions')
...
// --- Translation (comment/post "Translate" button) -------------------------
//
// Deliberately outside the isLive seam every other function above uses:
// translating text someone can already read needs no auth and touches no
// persisted state, so unlike posts/comments/follows it doesn't need a
// backend to be configured to work in a demo. When `isLive` (a real
// backend is deployed), this calls the rate-limited proxy at
// routes/translate.js. When it isn't, it calls the same public, keyless
// endpoint directly from the browser — same request/response shape
// either way, so callers never branch on isLive themselves.
export async function translateText(text, targetLang = 'en') {
  if (isLive) {
    return request('/translate', { method: 'POST', body: { text, targetLang }, auth: false })
  }
  const url =
  ...
  return { translatedText, detectedLang, targetLang }
}

// --- Live Settings (backend/src/routes/settings.js) -------------------------
//
// One row per user, fetched/updated as a whole object rather than one
// call per field — see the migration/route comments for why notifications/
// privacy/appearance/focus/muted_words live together. `updateSettings`
// sends only the categories actually changed; the backend merges each one
// into the existing row rather than requiring the full object every time.
